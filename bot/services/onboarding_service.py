...
class OnboardingService:
    ...
    async def ensure_verification_message(self, bot: discord.Client):
        ...
        try:
            ...
            logger.info(f"Verification message posted in channel {welcome_channel_id} (message ID: {message.id})")
    ...
    async def handle_user_join(self, member: discord.Member):
        ...
        if self.embed_logger:
            ...
            await self.embed_logger.log_custom(
                ...
                description=f"New member joined the server",
                level=LogLevel.INFO,
                fields={
        ...
        logger.info(f"New member joined: {member.name}#{member.discriminator} ({member.id}) - Account age: {account_age_days:.1f} days")
    ...
    async def handle_user_leave(self, member: discord.Member, was_verified: bool = None):
        ...
        if self.embed_logger:
            ...
            verification_status = "✅ Verified" if was_verified else "❌ Unverified" if was_verified is False else "❓ Unknown"
            ...
            await self.embed_logger.log_custom(
                ...
                fields={
                    ...
                    "User ID": str(member.id),
                    "Joined At": member.joined_at.strftime("%Y-%m-%d %H:%M UTC") if member.joined_at else "Unknown",
                    "Time in Server": str(discord.utils.utcnow() - member.joined_at).split('.')[0] if member.joined_at else "Unknown",
                    "Verification Status": verification_status,
                    "Roles": ", ".join([role.name for role in member.roles[1:]]) if len(member.roles) > 1 else "None"
                }
            )
        
        logger.info(f"Member left: {member.name}#{member.discriminator} ({member.id}) - Was verified: {was_verified}")
    ...
    async def get_verification_stats(self, bot: discord.Client) -> dict:
        ...
        stats = {
            ...
            "verification_rate": round((verified_count / human_members * 100), 1) if human_members > 0 else 0
        }
