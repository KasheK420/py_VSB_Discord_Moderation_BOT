...
class EmbedLogger:
    ...
    def __init__(self, bot: discord.Client, admin_channel_id: int):
        ...
        self.admin_channel: discord.TextChannel | discord.Thread | None = None
        ...
        self.rate_limit = {"messages": [], "max_per_minute": 30, "similar_message_cache": {}}
    ...
    async def setup(self) -> bool:
        ...
        if not self.admin_channel:
            logger.error(
                f"Admin log channel {self.admin_channel_id} not found (yet). Will retry once after ready."
            )
    ...
    async def _delayed_retry(self):
        ...
        try:
            ...
            if self.admin_channel:
                ...
                logger.error(
                    f"Admin log channel {self.admin_channel_id} still not found after retry."
                )
    ...
    def _should_rate_limit(self, content_hash: str) -> bool:
        ...
        self.rate_limit["messages"] = [
            msg_time
            for msg_time in self.rate_limit["messages"]
    ...
    async def _safe_send(self, embed: Embed, content_hash: str = None) -> discord.Message | None:
        """
    ...
    def _add_fields_to_embed(self, embed: Embed, fields: list[tuple] | dict[str, Any] | None):
    ...
    async def log_auth_start(self, user_id: str, username: str) -> int | None:
        """Log authentication start with pending status"""
        embed = self._create_base_embed(
            title="🔒 Authentication Started",
            description="User initiated VSB SSO authentication process",
            level=LogLevel.AUTH,
        )

        self._add_fields_to_embed(
            embed,
            {
    ...
    async def log_auth_success(self, user_id: str, user_info: dict[str, Any]):
        ...
        is_teacher = any(
            "employee" in str(g).lower() or "staff" in str(g).lower() or "faculty" in str(g).lower()
        ...
        embed = self._create_base_embed(
            ...
            description="User successfully verified via VSB SSO and granted server access",
            level=LogLevel.SUCCESS,
        )
        ...
        self._add_fields_to_embed(
            embed,
            {
                ...
                "Full Name": (
                    f"{first_name} {last_name}" if first_name and last_name else "Not provided"
                ),
        ...
        embed.set_footer(
            text=f"VSB SSO • Completed at {datetime.utcnow().strftime('%H:%M:%S UTC')}"
        )
    ...
    async def log_auth_failure(self, user_id: str, reason: str, error_details: str | None = None):
        ...
        if error_details:
            ...
            truncated_details = (
                error_details[:800] + "..." if len(error_details) > 800 else error_details
            )
            embed.add_field(
                name="Technical Details", value=f"```{truncated_details}```", inline=False
            )
    ...
    async def log_system_event(
        ...
        fields: list[tuple] | dict[str, Any] | None = None,
    ):
    ...
    async def log_service_event(
        ...
        embed = self._create_base_embed(
            title=f"[{service_name}] {event_type}", description=description, level=level
        )
        ...
        await self._safe_send(
            embed, f"service_{service_name}_{event_type}".lower().replace(" ", "_")
        )

    async def log_database_operation(
        self, operation: str, table: str, affected_rows: int = 0, details: str | None = None
    ):
        ...
        if details:
            ...
            safe_details = details.replace(
                self.bot.config.db_password if hasattr(self.bot, "config") else "", "[REDACTED]"
            )
            truncated_details = (
                safe_details[:500] + "..." if len(safe_details) > 500 else safe_details
            )
            embed.add_field(
                name="Query Details", value=f"```sql\n{truncated_details}\n```", inline=False
            )

        embed.set_footer(text="Database Monitor")
        await self._safe_send(embed, f"db_{operation}_{table}".lower())

    async def log_error(self, service: str, error: Exception, context: str | None = None):
        ...
        if tb and tb != "NoneType: None\n":
            ...
            embed.add_field(
                name="Traceback (tail)", value=f"```python\n{tb_short}\n```", inline=False
            )
    ...
    async def log_security_event(
        ...
        embed = self._create_base_embed(
            title=f"🛡️ Security Event - {event_type}", description=description, level=level
        )
    ...
    async def log_custom(
        ...
        embed = self._create_base_embed(
            title=f"[{service}] {title}", description=description, level=level
        )
    ...
    async def get_logging_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": round(
                (
                    self.stats["logs_sent"]
                    / max(1, self.stats["logs_sent"] + self.stats["logs_failed"])
                )
                * 100,
                2,
            ),
    ...
    async def log_stats_summary(self):
        ...
        await self.log_custom(
            ...
            fields={
                ...
                "Channel Status": (
                    "✅ Connected"
                    if stats["channel_status"]["channel_resolved"]
                    else "❌ Disconnected"
                ),
            },
        )
