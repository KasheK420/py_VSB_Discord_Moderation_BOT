...
from ..utils.ai_helper import ask_ai, moderate_message
...
class SmartModerationService:
    ...
    def analyze_text(self, text: str) -> tuple[int, list[str]]:
        ...
        if zalgo and len(zalgo.findall(text)) > self.config.get("limits", {}).get(
            "zalgo_threshold", 6
        ):
        ...
        if self.embed_logger:
            await self.embed_logger.log_system_event(
                ...
                fields=[
                    ...
                    (
                        "Daily AI Limit",
                        str(self.config.get("limits", {}).get("daily_ai_limit", 500)),
                        True,
                    ),
                    (
                        "Anti-Raid",
                        (
                            "Enabled"
                            if self.config.get("anti_raid", {}).get("enabled")
                            else "Disabled"
                        ),
                        True,
                    ),
                    (
                        ...
                        (
                            "Enabled"
                            if self.config.get("review_queues", {}).get("enabled")
                            else "Disabled"
                        ),
                        True,
                    ),
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, user: discord.User):
        ...
        while (
            self.user_reaction_limits[user_id]
            and current_time - self.user_reaction_limits[user_id][0] > 60
        ):
    ...
    async def _check_rate_limiting(self, message: discord.Message) -> bool:
        ...
        while (
            self.user_rate_limits[user_id] and current_time - self.user_rate_limits[user_id][0] > 10
        ):
    ...
    async def _analyze_message_content(self, message: discord.Message) -> tuple[int, list[str]]:
        ...
        if (email_pattern and email_pattern.search(content)) or (
            phone_pattern and phone_pattern.search(content)
        ):
        ...
        if zalgo_pattern and len(zalgo_pattern.findall(content)) > self.config.get(
            "limits", {}
        ).get("zalgo_threshold", 6):
            suspicion_score += weights.get("zalgo_text", 3)
            violations.append("zalgo_text")
    ...
    def _analyze_message_format(self, message: discord.Message, content: str) -> list[str]:
        ...
        # Mention analysis
        mention_count = (
            len(message.mentions) + len(message.role_mentions) + len(message.channel_mentions)
        )
    ...
    async def _handle_inappropriate_content(
        self, message: discord.Message, ai_result: dict, violations: list[str], suspicion_score: int
    ):
    ...
    def _determine_action(self, severity: str, warning_count: int, violations: list[str]) -> str:
        ...
        # Regular escalation
        if severity == "high" or warning_count >= escalation.get("severe_action", {}).get(
            "min_warnings", 3
        ):
    ...
    async def _send_warning_dm(self, message: discord.Message, warning: dict):
        """Send warning via DM"""
        template = self.config.get("actions", {}).get(
            "dm_warning_template",
        ...
        try:
            ...
            public_template = self.config.get("actions", {}).get(
                "public_warning_template", "{user}, please follow the rules. Reason: {reason}"
            )

            public_warning = public_template.format(
                user=message.author.mention, reason=", ".join(warning["violations"][:2])
            )
    ...
    async def _timeout_user(self, message: discord.Message, duration: timedelta, warning: dict):
        ...
        try:
            ...
            await message.author.timeout(
                until, reason=f"Auto-moderation: {', '.join(warning['violations'][:3])}"
            )
    ...
    async def _ban_user(self, message: discord.Message, warning: dict):
        ...
        try:
            await message.author.ban(
                reason=f"Auto-moderation: {', '.join(warning['violations'])}", delete_message_days=1
            )
            await message.channel.send(
            ...
            if self.embed_logger:
                await self.embed_logger.log_custom(
                    ...
                    description="User automatically banned for severe violations",
                    level=LogLevel.CRITICAL,
                    fields={
                        "User": f"{message.author.name}#{message.author.discriminator} ({message.author.id})",
                        "Violations": ", ".join(warning["violations"]),
                        "Message Content": (
                            warning["message_content"][:100] + "..."
                            if len(warning["message_content"]) > 100
                            else warning["message_content"]
                        ),
                        "Channel": message.channel.mention,
    ...
    async def _log_moderation_incident(self, message: discord.Message, warning: dict, action: str):
        ...
        severity_colors = {"low": LogLevel.INFO, "medium": LogLevel.WARNING, "high": LogLevel.ERROR}
        ...
        await self.embed_logger.log_custom(
            ...
            fields={
                ...
                "AI Reason": (
                    warning["ai_reason"][:100] + "..."
                    if len(warning["ai_reason"]) > 100
                    else warning["ai_reason"]
                    if warning["ai_reason"]
                    else "N/A"
                ),
                "Channel": message.channel.mention,
    ...
    async def _handle_raid_detection(self, guild: discord.Guild):
        ...
        if self.embed_logger:
            await self.embed_logger.log_custom(
                ...
                title="🚨 Raid Detected",
                description="Burst join threshold exceeded - automatic countermeasures activated",
                level=LogLevel.CRITICAL,
                fields={
        ...
        if account_age_hours < 1:  # Less than 1 hour old
            ...
            if self.embed_logger:
                await self.embed_logger.log_custom(
                    ...
                    description="Extremely new account detected - potential risk",
                    level=LogLevel.WARNING,
                    fields={
    ...
    async def _handle_duplicate_spam(self, message: discord.Message):
        ...
        try:
            await message.channel.send(
                f"{message.author.mention} please don't repeat the same message.", delete_after=15
            )
        except Exception:
            pass
        ...
        try:
            ...
            if member:
                await member.timeout(
                    datetime.utcnow() + timedelta(minutes=2), reason="Reaction spam"
                )
    ...
    async def _queue_for_manual_review(
        self, message: discord.Message, violations: list[str], score: int
    ):
        ...
        if self.embed_logger:
            await self.embed_logger.log_custom(
                ...
                description="High suspicion content requires manual review",
                level=LogLevel.WARNING,
                fields={
                    "User": f"<@{message.author.id}>",
        ...
        while True:
            ...
            for user_id in list(self.user_rate_limits.keys()):
                while (
                    ...
                    and current_time - self.user_rate_limits[user_id][0] > 600
                ):  # 10 minutes
                    self.user_rate_limits[user_id].popleft()
                    cleaned_items += 1
            ...
            for user_id in list(self.user_reaction_limits.keys()):
                while (
                    self.user_reaction_limits[user_id]
                    and current_time - self.user_reaction_limits[user_id][0] > 3600
                ):  # 1 hour
                    self.user_reaction_limits[user_id].popleft()
                    cleaned_items += 1
    ...
    def get_moderation_stats(self) -> dict:
        ...
        active_users_tracked = len(
            [
                uid
                for uid, warnings in self.user_warnings.items()
            ...
            asyncio.create_task(
                self.embed_logger.log_custom(
                    ...
                    fields={
                        ...
                        "New AI Calls": "0",
                        "Daily Limit": str(
                            self.config.get("limits", {}).get("daily_ai_limit", 500)
                        ),
        ...
        if user_id in self.user_warnings:
            ...
            if self.embed_logger:
                await self.embed_logger.log_custom(
                    ...
                    description="All warnings cleared for user",
                    level=LogLevel.INFO,
                    fields={
                        "User": f"<@{user_id}>",
        ...
        if 0 <= item_id < len(self.review_queue):
            ...
            if self.embed_logger:
                await self.embed_logger.log_custom(
                    ...
                    description="Review queue item processed by moderator",
                    level=LogLevel.INFO,
                    fields={
