...
class AuthService:
                ...
                if self.embed_logger:
                    await self.embed_logger.log_custom(
                        ...
                        fields={
                            ...
                            "Verified Since": existing_user.verified_at.strftime("%Y-%m-%d %H:%M") if existing_user.verified_at else "Unknown",
    ...
    def generate_cas_login_url(self, discord_user_id: str) -> str:
        ...
        if self.embed_logger:
            asyncio.create_task(self.embed_logger.log_custom(
                ...
                description=f"CAS login URL generated for user",
                level=LogLevel.INFO,
                fields={
    ...
    async def process_cas_callback(self, ticket: str, state: str) -> dict:
        ...
        if self.embed_logger:
            await self.embed_logger.log_custom(
                ...
                description=f"Processing CAS callback for user",
                level=LogLevel.INFO,
                fields={
                    "User": f"<@{discord_user_id}>",
                    "Ticket": ticket[:8] + "...",
                    "State": state[:8] + "...",
                    "Auth Age": str((datetime.utcnow() - auth_info['timestamp']).total_seconds()) + "s"
                }
            )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(validate_url) as response:
                if response.status != 200:
                    ...
                    error = Exception(f"CAS validation failed (HTTP {response.status}): {error_text}")
        ...
        try:
            ...
            if success is None:
                ...
                if failure is not None:
                    ...
                    if self.embed_logger:
                        asyncio.create_task(self.embed_logger.log_error(
                            service="Authentication",
                            error=error,
            ...
            if self.embed_logger:
                asyncio.create_task(self.embed_logger.log_custom(
                    ...
                    description=f"Successfully parsed user data from CAS",
                    level=LogLevel.SUCCESS,
                    fields={
                        "Username": username,
                        "Attributes Found": str(len(attributes)),
                        "Email": attributes.get('mail', 'not provided')[:50],
                        "Groups": str(len(attributes.get('groups', '').split(',') if attributes.get('groups') else [])) + " groups"
                    }
                ))
            ...
            return {
                ...
                'eduPersonAffiliation': attributes.get('eduPersonAffiliation', '').split(',') if attributes.get('eduPersonAffiliation') else []
            }
                ...
                if self.embed_logger:
                    asyncio.create_task(self.embed_logger.log_custom(
                        ...
                        fields={
                            ...
                            "All Groups": ", ".join(groups[:5]) + ("..." if len(groups) > 5 else "")
                        }
                    ))
                return 2
    ...
    async def save_user(self, discord_id: str, login: str, email: str, 
                       first_name: str, last_name: str, user_type: int, 
                       attributes: dict):
        """Save or update user in database"""
        user = User(
            id=discord_id,
            ...
            try:
                ...
                if self.embed_logger:
                    await self.embed_logger.log_custom(
                        ...
                        title="Role Assigned",
                        description=f"Successfully assigned role to verified user",
                        level=LogLevel.SUCCESS,
                        fields={
                            "User": f"<@{discord_user_id}>",
                            "Role": role.name,
