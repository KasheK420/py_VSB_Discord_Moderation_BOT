...
class ShameQueries:
    ...
    async def ensure_schema(pool: asyncpg.Pool) -> None:
        async with pool.acquire() as conn:
            ...
            """)
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_shame_user_at ON shame_events(user_id, at desc);")

    @staticmethod
    async def _ensure_row(conn: asyncpg.Connection, user_id:int) -> None:
        await conn.execute("INSERT INTO shame_stats(user_id) VALUES($1) ON CONFLICT (user_id) DO NOTHING", user_id)

    @staticmethod
    async def add_event(pool: asyncpg.Pool, *, user_id:int, kind:str, reason:str|None, moderator_id:int|None) -> None:
        ...
        async with pool.acquire() as conn:
            ...
            VALUES ($1,$2,$3,$4)
            """, user_id, kind, reason, moderator_id)
    ...
    @staticmethod
    async def leaderboard(pool: asyncpg.Pool, by:str="warnings", limit:int=10) -> List[Dict[str,Any]]:
