...
class DataMigrationService:
    ...
    def _read_dump_file(self, dump_file_path: Path) -> str | None:
        ...
        try:
            ...
            try:
                with open(dump_file_path, encoding="latin-1") as f:
                    content = f.read()
    ...
    def _convert_mysql_to_postgresql(self, mysql_sql: str) -> str:
        ...
        pg_sql = re.sub(
            r"(\w+)\s+INT\s+NOT\s+NULL\s+AUTO_INCREMENT", r"\1 SERIAL", pg_sql, flags=re.IGNORECASE
        )
        ...
        pg_sql = re.sub(
            r"ON\s+DUPLICATE\s+KEY\s+UPDATE.*?;", ";", pg_sql, flags=re.IGNORECASE | re.DOTALL
        )
            ...
            if "duplicate key" in error_msg or "already exists" in error_msg:
                ...
                if "INSERT INTO" in statement.upper():
                    ...
                    table_match = re.search(
                        r'INSERT\s+INTO\s+["`]?(\w+)["`]?', statement, re.IGNORECASE
                    )
                    if table_match:
