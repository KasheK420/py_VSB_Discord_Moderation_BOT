...
def _normalize(loc: str | None) -> str | None:
    if not loc:
        return None
    loc = loc.strip()
    return _LOCALE_MAP.get(loc) or _LOCALE_MAP.get(loc.split("-")[0], None)
...
def language_directive(lang: str) -> str:
    ...
    return (
        f"Always answer in {pretty} unless the user explicitly requests another language. "
