...
class AdminCog(commands.Cog):
    ...
    async def mod_reload(self, interaction: Interaction):
        ...
        if not mod:
            ...
            return await interaction.followup.send("❌ Moderation service unavailable.", ephemeral=True)
    ...
    async def mod_stats(self, interaction: Interaction):
        ...
        text = "\n".join(f"**{k.replace('_',' ').title()}**: {v}" for k, v in stats.items()) or "No stats available."
    ...
    async def mod_reset_daily(self, interaction: Interaction):
        ...
        if not mod:
            ...
            return await interaction.response.send_message("❌ Moderation service unavailable.", ephemeral=True)
    ...
    @app_commands.command(name="sync_commands", description="Force re-sync of all slash/context commands")
    @app_commands.checks.has_permissions(administrator=True)
    async def sync_commands(self, interaction: Interaction, scope: str = "guild"):
        ...
        try:
            if scope.lower() == "guild":
                ...
                await interaction.followup.send(f"✅ Synced {len(synced)} global commands (propagation can take ~1h)", ephemeral=True)
        except Exception as e:
            if self.embed_logger:
                await self.embed_logger.log_error(
                    service="Admin Commands",
    ...
    @app_commands.command(name="mod_config", description="Show the current moderation.json (paginated)")
    @app_commands.checks.has_permissions(administrator=True)
    async def mod_config(self, interaction: Interaction):
        ...
        if not mod or not getattr(mod, "config", None):
            ...
            return await interaction.followup.send("❌ Moderation service not ready or config missing.", ephemeral=True)
        ...
        def page(title: str, data: str) -> discord.Embed:
            ...
            if len(data) <= chunk_size:
                ...
                for i in range(0, len(data), chunk_size):
                    e.add_field(name=f"Part {i//chunk_size + 1}", value=f"```json\n{data[i:i+chunk_size]}\n```", inline=False)
            return e
        ...
        if isinstance(bad, dict) and bad:
            counts = {k: (len(v) if isinstance(v, list) else (len(v) if v else 0)) for k, v in bad.items()}
        ...
        # Safety if no pages
        if not pages:
            pages = [discord.Embed(title="Moderation Config", description="(empty)", color=discord.Color.blurple())]
    ...
    async def log_test(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("Sent a test embed to the admin log channel.", ephemeral=True)
        except Exception as e:
...
class PagedEmbedView(View):
    ...
    async def prev(self, interaction: discord.Interaction, _):
        if interaction.user.id != self.author_id:
            return await interaction.response.send_message("❌ Only the requester can control pagination.", ephemeral=True)
        if self.index > 0:
            self.index -= 1
    ...
    async def close(self, interaction: discord.Interaction, _):
        if interaction.user.id != self.author_id:
            return await interaction.response.send_message("❌ Only the requester can close this.", ephemeral=True)
        for child in self.children:
            child.disabled = True
