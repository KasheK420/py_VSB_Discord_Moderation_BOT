...
# Setup logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - [DEV] - %(message)s", datefmt="%H:%M:%S"
)
        ...
        with self.restart_lock:
            ...
            try:
                ...
                if self.bot_process and self.bot_process.poll() is None:
                    ...
                    output_thread = threading.Thread(target=self.read_bot_output, daemon=True)
                    output_thread.start()
...
╚══════════════════════════════════════════════╝
        ...
        # Watch config files
        config_files = [Path("config.json"), Path("auth.json"), Path(".env"), Path(".env.dev")]
